...
public class ViewBuyTicketPageCommand extends Command {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response)
		...
		if (station1 != null && !station1.isEmpty() && station2 != null && !station2.isEmpty() && date != null
				&& !date.isEmpty() && idStr != null && !idStr.isEmpty() && station2.matches("[0-9]*")
				&& station1.matches("[0-9]*") && idStr.matches("[0-9]*") && type != null && !type.isEmpty()
			...
			if (((Client) session.getAttribute("client")) != null) {
				...
				if (entredDate.getTime() < currectDate.getTime()) {
					...
					if (seats >= 1) {
						...
						booking.setArrivalTime(new Timestamp(
								booking.getDepartureTime().getTime() - (long) (travelTime + stopTime) * 60000L));
