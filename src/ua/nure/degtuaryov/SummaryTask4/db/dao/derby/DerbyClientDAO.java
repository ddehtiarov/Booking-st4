...
public class DerbyClientDAO implements ClientDAO {
	...
	public List<Client> findAllClients() {
		...
		try {
			...
			Collections.sort(result, new Comparator<Client>() {
				@Override
				public int compare(Client cl1, Client cl2) {
					return -cl1.getTotalPrice() + cl2.getTotalPrice();
				}
			});
	...
	private Client obtainClient(ResultSet resultSet) throws SQLException {
		...
		int totPrice = 0;
		List<Booking> bookings = daoFactory.getBookingDAO().fingAllBookingByClientId(resultSet.getLong(Fields.ENTITY_ID));
		for(Booking booking : bookings){
			totPrice += booking.getPrice();
		}
		client.setTotalPrice(totPrice);
		return client;
	}
