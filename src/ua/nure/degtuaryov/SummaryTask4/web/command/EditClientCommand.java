...
public class EditClientCommand extends Command {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getAttribute("id") == null || request.getAttribute("id").toString().isEmpty()) {
			...
			if (request.getParameter("sale") != null && !request.getParameter("sale").isEmpty()) {
				int sale = Integer.valueOf(request.getParameter("sale"));
				if (sale != 0) {
					java.util.Date currectDate = new java.util.Date();
					List<Booking> list = daoFactory.getBookingDAO().fingAllBookingByClientId(id);
					for (Booking booking : list) {
						if (booking.getArrivalTime().after(currectDate)) {
							int newP = booking.getPrice() - booking.getPrice() * sale / 100;
							booking.setPrice(newP);
							daoFactory.getBookingDAO().updateBooking(booking);
						}
					}
				}
			}
